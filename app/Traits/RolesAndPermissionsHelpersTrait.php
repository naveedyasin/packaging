...
trait RolesAndPermissionsHelpersTrait
{
    ...
    public function getDeletedPermission($id)
    {
        ...
        if (count($item) != 1) {
            return abort(
                redirect('laravelroles::roles.index')
                    ->with(
                        'error',
                        trans('laravelroles::laravelroles.errors.errorDeletedPermissionNotFound')
                    )
            );
        }
        ...
        if (count($item) != 1) {
            return abort(
                redirect('laravelroles::roles.index')
                    ->with(
                        'error',
                        trans('laravelroles::laravelroles.errors.errorDeletedRoleNotFound')
                    )
            );
        }
    ...
    public function getPermissionModels()
    {
        ...
        return DB::table(config('roles.permissionsTable'))->pluck('model')->merge(
            collect(class_basename(new $permissionModel()))
        )->unique();
    }
...
//            'sortedPermissionsRolesUsers' => $sortedPermissionsRolesUsers,
     ...
     */
    public function retrievePermissionRoles(
        $permission,
        $permissionsAndRolesPivot,
        $sortedRolesWithUsers
    ) {
        $roles = [];
     ...
     */
    public function retrievePermissionUsers(
        $permission,
        $permissionsAndRolesPivot,
        $sortedRolesWithUsers,
        $permissionUsersPivot,
        $appUsers
    ) {
        $users = [];
        $userIds = [];
        ...
        foreach ($permissionUsersPivot as $permissionUsersPivotKey => $permissionUsersPivotItem) {
            if (!in_array(
                    $permissionUsersPivotItem->admin_id,
                    $userIds
                ) && $permission->id === $permissionUsersPivotItem->permission_id) {
     ...
     */
    public function getSortedPermissonsWithRolesAndUsers(
        $sortedRolesWithUsers,
        $permissions,
        $users
    ) {
        ...
        foreach ($permissions as $permissionKey => $permissionValue) {
            $sortedPermissionsWithRoles[] = [
                ...
                'roles' => $this->retrievePermissionRoles(
                    $permissionValue,
                    $permissionsAndRolesPivot,
                    $sortedRolesWithUsers
                ),
                'users' => $this->retrievePermissionUsers(
                    $permissionValue,
                    $permissionsAndRolesPivot,
                    $sortedRolesWithUsers,
                    $permissionUsersPivot,
