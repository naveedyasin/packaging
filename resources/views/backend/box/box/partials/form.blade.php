    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="form-group">
                <label>Title:</label>
                <input type="text" class="form-control" name="title" value="{{old('title', $box->title)}}" placeholder="Post title" required>
            </div>

            <div class="form-group">
                <label>Contents:</label>
                <textarea id="summernote" name="body" class="form-control">{{old('body', $box->body)}}</textarea>
            </div>

            <div class="text-right">
                <a href="{{route('admin.box.index')}}" class="btn btn-default">Back</a>
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <select class="form-control select" data-fouc name="status">
                    <option value="published" @if($box->status == 'published') selected @endif>Published</option>
                    <option value="pending" @if($box->status == 'pending') selected @endif>Pending</option>
                    <option value="draft" @if($box->status == 'draft') selected @endif>Draft</option>
                </select>
            </div>
        </div>
    </div>
                ...
                <select multiple name="categories[]" id="categories" class="form-control select" data-fouc>
                    @if($categories->count())
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ is_array(old('categories')) ? (in_array($category->id, old('categories')) ? 'selected' : '') : (in_array($category->id, $boxCategories) ? 'selected' : '') }}>{{$category->name}}</option>
                        @endforeach
                    @endif
                </select>
            </div>
    ...
    <div class="card card-collapsed">
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <textarea name="excerpt" id="excerpt" cols="30" rows="10" class="form-control">{{ old('excerpt',$box->excerpt) }}</textarea>
            </div>
        </div>
    </div>
    <div class="card card-collapsed">
            ...
            <div class="form-group">
                <div class="form-check">
                    <label class="form-check-label">
                        <input type="checkbox" name="allow_comments" id="allow_comments" class="form-check-input-styled" {{old('allow_comments') == 'on' || $box->allow_comments ? 'checked' : ''}} data-fouc>
                        Allow Comments
                    </label>
                </div>
            </div>
